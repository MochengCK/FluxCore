...
{
  A2_LOG_DEBUG(fmt("CUID#%" PRId64 " - ConnectCommand::executeInternal called",
                   getCuid()));
  ...
  A2_LOG_DEBUG(fmt("CUID#%" PRId64 " - Checking if connection established",
                   getCuid()));
  ...
  if (!checkIfConnectionEstablished(
    ...
    A2_LOG_DEBUG(fmt("CUID#%" PRId64 " - Connection not yet established, waiting",
                     getCuid()));
    return true;
  }
  
  A2_LOG_INFO(fmt("CUID#%" PRId64 " - Connection established, running chain",
                  getCuid()));
