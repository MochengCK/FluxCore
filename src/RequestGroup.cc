...
#ifdef ENABLE_BITTORRENT
  // 保存封禁列表到文件（作为安全措施）
  // 使用session目录而不是下载目录，避免在用户文件夹中显示配置文件
  if (peerStorage_ && option_ && gid_) {
    auto defaultPeerStorage = dynamic_cast<DefaultPeerStorage*>(peerStorage_);
    if (defaultPeerStorage) {
      try {
        // 优先使用 --dir 选项指定的目录，如果没有则使用当前目录
        std::string sessionDir = option_->get(PREF_DIR);
        
        // 如果有 save-session 选项，使用其父目录
        ...
        std::string bannedPeersFile = util::applyDir(configDir, gid_->toHex() + ".banned");
        
        A2_LOG_INFO(fmt("~RequestGroup: Saving banned peers for GID#%s to %s", 
                        gid_->toHex().c_str(), bannedPeersFile.c_str()));
        ...
        defaultPeerStorage->saveBannedPeers(bannedPeersFile);
      } catch (...) {
        // 忽略析构函数中的异常
        A2_LOG_WARN(fmt("~RequestGroup: Failed to save banned peers for GID#%s", 
                        gid_->toHex().c_str()));
      }
    }
  }
    ...
    std::vector<std::unique_ptr<Command>>& commands, DownloadEngine* e)
{
  ...
  if (downloadContext_->hasAttribute(CTX_ATTR_BT)) {
    ...
    A2_LOG_INFO(fmt("Loading banned peers for GID#%s from %s", 
                    gid_->toHex().c_str(), bannedPeersFile.c_str()));
    ...
    if (defaultPeerStorage) {
      ...
      for (const auto& entry : badPeers) {
        A2_LOG_INFO(fmt("  Banned IP: %s", entry.first.c_str()));
      }
    }
...
void RequestGroup::releaseRuntimeResource(DownloadEngine* e)
{
  ...
  if (peerStorage_) {
    ...
    if (defaultPeerStorage) {
      ...
      A2_LOG_INFO(fmt("Saving banned peers for GID#%s to %s", 
                      gid_->toHex().c_str(), bannedPeersFile.c_str()));
