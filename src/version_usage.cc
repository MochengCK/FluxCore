...
{
  out->printf(_("Usage: xfercore [OPTIONS] [URI | MAGNET | TORRENT_FILE |"
                " METALINK_FILE]..."));
  out->printf("\n");
  if (keyword.empty()) {
    // Very short version of usage.
    out->printf(_("See 'xfercore -h'."));
    out->printf("\n");
    return;
  }
  else if (keyword[0] == '#') {
    ...
    if (keyword == STR_TAG_ALL) {
      ...
      out->printf("\n");
      out->printf(_("See 'xfercore -h#help' to know all available tags."));
    }
    out->printf("\n");
  ...
  if (keyword == strHelpTag(TAG_BASIC)) {
    ...
    out->printf(
        _("About the number of connections\n"
          " Since 1.10.0 release, XferCore uses 1 connection per host by default "
          "and has 20MiB\n"
