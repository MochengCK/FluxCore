...
#  if OPENSSL_VERSION_NUMBER >= 0x30000000L
  ...
  // Make legacy provider optional - it's only needed for old algorithms
  legacy_provider_ = OSSL_PROVIDER_load(nullptr, "legacy");
  if (!legacy_provider_) {
    // Log warning but don't fail - legacy algorithms are rarely needed
    std::cerr << "Warning: Failed to load OpenSSL legacy provider. "
              << "Old algorithms (RC4, MD5, etc.) will not be available." << std::endl;
  }
