...
std::unique_ptr<RpcMethod> createMethod(const std::string& methodName)
{
  ...
  if (methodName == GetTaskStatsRpcMethod::getMethodName()) {
    return make_unique<GetTaskStatsRpcMethod>();
  }

  if (methodName == BanPeerRpcMethod::getMethodName()) {
    return make_unique<BanPeerRpcMethod>();
  }
  ...
  if (methodName == GetBtLevelRpcMethod::getMethodName()) {
    return make_unique<GetBtLevelRpcMethod>();
  }
