...
SelectEventPoll::SelectEventPoll()
{
  ...
  // WSAStartup returns 0 on success, or non-zero if already initialized (which is OK)
  (void)wsaResult; // Suppress unused variable warning
...
SelectEventPoll::~SelectEventPoll()
{
  ...
  // Note: We don't call WSACleanup() here because WSAStartup() may have been
  // called elsewhere (e.g., in Platform::init()), and we don't want to
  // interfere with that. The OS will clean up Winsock when the process exits.
#endif // __MINGW32__
}
