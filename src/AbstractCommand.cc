    ...
    uint16_t connectedPort)
{
  ...
  A2_LOG_DEBUG(fmt("checkIfConnectionEstablished: hostname=%s, addr=%s, port=%d, error='%s'",
                   connectedHostname.c_str(), connectedAddr.c_str(), 
                   connectedPort, error.c_str()));
  
  if (error.empty()) {
    A2_LOG_DEBUG("checkIfConnectionEstablished: Connection established successfully");
    return true;
  }

  A2_LOG_INFO(fmt("checkIfConnectionEstablished: Connection failed with error: %s",
                  error.c_str()));
